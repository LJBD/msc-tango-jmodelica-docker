...
RUN yum --disablerepo=SOL* --disablerepo=max* install -y Cython subversion java zlib-devel lapack-devel blas-devel gcc-c++ gcc-gfortran cmake swig python2-jpype metis64-devel numpy-f2py wget patch make which ant
